        ...
        if r.returncode == 0:
            ...
            for line in r.stdout.split("\n"):
                if "NVIDIA" in line and "MiB" not in line and "Driver" not in line:
                    ...
                    if len(gpu) >= 2:
                        print(f"  V GPU: {gpu[1].strip()}")
                        break
            return True
        else:
            print("  X nvidia-smi 执行失败")
            return False
    except FileNotFoundError:
        print("  X nvidia-smi 未找到")
        print("")
        print("  +---------------------------------------------+")
...
def check_python():
    ...
    if ver.major == 3 and ver.minor == 12:
        print(f"  V Python 3.12，直接使用")
        return get_system_python()

    # 查找系统中的 3.12
    py312 = find_python312()
    if py312:
        print(f"  V 找到 Python 3.12: {py312}")
        return py312
    ...
    if platform.system() == "Windows":
        ...
        if ok:
            py312 = find_python312()
            if py312:
                print(f"  V Python 3.12 安装成功: {py312}")
                return py312
...
def install_cuda_pytorch(venv_py):
    ...
    if len(parts) == 3:
        ...
        print(f"  V PyTorch {ver} (CUDA {cuda}, available={avail})")
        if avail == "True":
            r2 = subprocess.run(
                [str(venv_py), "-c",
            ...
            print(f"  V GPU: {r2.stdout.strip()}")
        return avail == "True"

    print("  X PyTorch 验证失败")
    return False
...
def main(auto=False):
    ...
    print(f"  CUDA: {'V 就绪' if cuda_ok else 'X 不可用'}")
    if not driver_ok:
    ...
    if auto:
        if cuda_ok:
            ...
            print("VENV_READY=0 (CUDA 不可用但不影响启动)")
            sys.exit(0)
    else:
        input("\n按回车退出...")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--auto", action="store_true", help="自动模式（跳过交互确认）")
